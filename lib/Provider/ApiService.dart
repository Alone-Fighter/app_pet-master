...
import 'package:app_pet/Model/user.dart';
...
import '../main.dart';
import '../vet_screens/vet_reg2.dart';
...
class ApiService extends ChangeNotifier {
  ...
  bool vet = false;
  int eventsMonth=DateTime.now().month;
  String selectedPet = '';
  String petId = '';
  String petType = 'Dog';
  String breed = 'Raza';
  ...
  bool isselectdate1 = false;
  bool isselecttime1 = false;
  ...
  late ApiStatusLoginNew apistatuslogin;
  late ApiStatusSignUp apistatussignup;
  late ApiStatusSignUpVet apistatussignupvet;
  late ApiStatusSignUpVet2 apiStatusSignUpVet2;
  ...
  TextEditingController petSizeController = TextEditingController();
  ...
  TextEditingController get getPetSexController => petSexController;
  TextEditingController get getPetSizeController => petSizeController;
  TextEditingController get getPetBreedController => petBreedController;
  TextEditingController get getPetEditYearController => editYearController;
  TextEditingController get getPetEditMonthController => editMonthController;
  TextEditingController get getPetEditDayController => editDayController;
...
//----------------------------vet-----------------------------------------------
  ...
  setSignUpVet(String val) => SignUpVet = val;
  ...
  setDesDate(String day,month,year,inter,datetime,isseldate,isseltime){
    desDay = day;
    desMonth = month;
    desYear = year;
    desInter = inter;
    datetime1 = datetime;
    isselectdate1 = isseldate;
    isselecttime1 = isseltime;

  }
  setDesEDate(String day,month,year,extern,datetime,isseldate,isseltime){
    desDay2 = day;
    desMonth2 = month;
    desYear2 = year;
    desExtern = extern;
  ...
  apiListenerLogin(ApiStatusLoginNew apiStatus) {
    this.apistatuslogin = apiStatus;
  }
  apiListenerUpload(UploadStatus uploadStatus) {
    this.uploadStatus = uploadStatus;
  }
  apiListenerSignUp(ApiStatusSignUp apiStatusSignUp) {
    this.apistatussignup = apiStatusSignUp;
  }
  apiListenerSignUpVet(ApiStatusSignUpVet apiStatusSignUpVet){
    this.apistatussignupvet = apiStatusSignUpVet;
  }
  apiListenerSignUpVet2(ApiStatusSignUpVet2 apiStatusSignUpVet2){
    this.apiStatusSignUpVet2 = apiStatusSignUpVet2;
  }
  ...
  Future<void> EditAllPets(id){
    Map<String,dynamic> map = {};
      map['internos'] = desInter;
      map['year1'] = desYear;
    return fs.collection('pets').doc(myUser).collection('pet').doc(id).update(map);

  }

  Stream<QuerySnapshot> getAllHistory(petId , mode){
    return fs.collection('pets').doc(myUser).collection('pet').doc(petId).collection(mode).orderBy('time').snapshots();

  }
  Stream<QuerySnapshot> getAllHistorydes(petId , mode,wh){
    print('time$wh');
    return fs.collection('pets').doc(myUser).collection('pet').doc(petId).collection(mode).orderBy('time$wh').snapshots();
  ...
  Stream<QuerySnapshot> getProduct4(){
    return fs.collection('products').doc('Zl6gYmn3Z08AaH6AK6Xg').collection('logo').orderBy('name').snapshots();
  ...
  Future<DocumentSnapshot<Map<String, dynamic>>> getAllVet2(){
    return fs.collection('vet').doc(myUser).get();
  ...
  Stream<QuerySnapshot>getCalendarEvent(){
    return fs.collection('pets').doc(myUser).collection('pet').doc(getSelectedPet).collection('calendar').orderBy('month').orderBy('day').snapshots();
  ...
  updateProfilePetDetails( String petId,context) async {

      Map<String,dynamic> map = {};
      if(getPetBreedController.text.isNotEmpty){
        map['breed'] = getPetBreedController.text;
      }
      if(getPetSexController.text.isNotEmpty){
        map['sex'] = getPetSexController.text;
      }
      if(getPetSizeController.text.isNotEmpty){
        map['size'] = getPetSizeController.text;
      }
      if(getPetNameController.text.isNotEmpty){
        map['name'] = getPetNameController.text;
      }
      if(getPetEditYearController.text.isNotEmpty){
        map['year'] = getPetEditYearController.text;
      }
      if(getPetEditMonthController.text.isNotEmpty){
        map['month'] = getPetEditMonthController.text;
      }
      if(getPetEditDayController.text.isNotEmpty){
        map['day'] = getPetEditDayController.text;
      }
      fs.collection('pets').doc(myUser).collection('pet').doc(petId).update(map).whenComplete((){
        kNavigatorBack(context);
        getPetBreedController.text='';
        getPetSexController.text='';
        getPetSizeController.text='';
        getPetNameController.text='';
        getPetEditYearController.text='';
        getPetEditMonthController.text='';
        getPetEditDayController.text='';
  ...
  updateProfileOwnerDetails(context) async {

    Map<String,dynamic> map = {};
    if(getFullNameController.text.isNotEmpty){
      map['name'] =getFullNameController.text;
    }
    if(getEmailController.text.isNotEmpty){
      map['email'] = getEmailController.text;
    }
    if(getNumberController.text.isNotEmpty){
      map['number'] = getNumberController.text;
    }

    fs.collection('users').doc(myUser).update(map).whenComplete((){
      kNavigatorBack(context);
      getFullNameController.text='';
      getEmailController.text='';
      getNumberController.text='';
  ...
  updateDesparasitacionImage(String petId) async {
    ...
    if(isselectdate1 == true && isselecttime1 == true ){
        map['internos'] = desInter;
        map['year1'] = desYear;
        map['month1'] = desMonth;
        map['day'] = desDay;
      ...
      if(isselectdate2 == true && isselecttime2 == true ){
      ...
      if(map.isNotEmpty){
        if(isselectdate1 == true && isselecttime1 == true){
          setCalendarItem('desparasitacion', int.parse(desYear) , int.parse(desMonth) , int.parse(desDay),'');

        };
        if(isselectdate2 == true && isselecttime2 == true){
          setCalendarItem('desparasitacion', int.parse(desYear2) , int.parse(desMonth2) , int.parse(desDay2),'');
       ...
       if(recordPath.isNotEmpty){
         ...
         fs.collection('pets').doc(myUser).collection('pet').doc(petId).collection('vaccine').doc().set(map).whenComplete((){
          if(vacYear.length>1){
            ...
            setCalendarItem('Vacunas', Ryear , Rmonth , Rday,'');
          }
           uploadStatus.uploaded();
           resetUpdate();
           setLoading(false);
        ...
        if( recordPath.isNotEmpty){
          ...
          fs.collection('pets').doc(myUser).collection('pet').doc(petId).collection('visit').doc().set(map).whenComplete((){
            if(vacYear.length>1){
              ...
              var Rmonth=int.parse(vacMonth);
              var Rday=int.parse(vacDay);
              setCalendarItem('Visita al veterinario', Ryear , Rmonth , Rday,'');
  ...
  Stream<QuerySnapshot> getCalendarItem(){
    return fs.collection('pets').doc(myUser).collection('pet').doc(getSelectedPet).collection('calendar').orderBy('day').snapshots();
    //   value.docs.map((e){
    //     print(e.data()['mode']);
    //     list.add(e.data());
    //   }).toList();
  ...
  setCalendarItem(String mode , int? year,int? month , int? day,String description) {
  ...
  editCalendarItem(String mode , int? year,int? month , int? day,String description,documentId) {
    Map<String,dynamic> data = {};
    ...
    fs.collection('pets').doc(myUser).collection('pet').doc(getSelectedPet).collection('calendar').doc(documentId).update(data);
  ...
  bool checkSignUpValidate(String city){
    ...
    if(email.isNotEmpty ||
      ...
      if(fullNameController.text.isEmpty){
        apistatussignup.inputEmptyName();
        return false;
      }
      else if(numberController.text.isEmpty){
        apistatussignup.inputEmptyNumber();
        return false;

      }
      else if(emailController.text.isEmpty){
        apistatussignup.inputEmptyEmail();
        return false;
        ...
        apistatussignup.inputWrongEmail();
        return false;

      }
      else if(city.isEmpty){
        apistatussignup.inputEmptyCity();
        return false;

      }
      else if(year == '0' || month == '0'|| day == '0' || day == 'Dia' || month == 'Mes' || year == 'Año'){
        apistatussignup.inputEmptydate();
        return false;

      }
      else if(password1Controller.text.isEmpty){
        apistatussignup.inputEmptyPass1();
        return false;

      }
      else if(password2Controller.text.isEmpty){
        apistatussignup.inputEmptyPass2();
        return false;
      }
      if(password1 == password2){
        return true;
      }else{
        apistatussignup.inputWrongverifypass();
        return false;
      }
    }else{
      return false;
  ...
  bool checkSignUpValidateVet(){
    ...
    String email = getVetEmailController.text;
    String password1 = getVetPassword1Controller.text;
    String password2 = getVetPassword2Controller.text;
    String number = getVetNumberController.text;
    String nameproperty = getVetPropertyController.text;
    String direction = getVetDirectionController.text;
    String nit = getVetNitController.text;
    String professional = getVetProfessionalCard.text;
    if(SignUpVet == '1'){
      if(name.isNotEmpty ||
        ...
        if(name.isEmpty){
          apistatussignupvet.inputEmptyName();
          return false;
        }
        else if(number.isEmpty){
          apistatussignupvet.inputEmptyNumber();
          return false;

        }
        else if(vetCityShop.isEmpty){
          print(vetCity);
          apistatussignupvet.inputEmptyCity();
          return false;

        }
        else if(expert.isEmpty){
          apistatussignupvet.inputEmptySpecialty();
          return false;

        }
        else if(professional.isEmpty){
          apistatussignupvet.inputEmptyProfessional();
          return false;

        }
        else if(workAs.isEmpty){
          apistatussignupvet.inputEmptyWorkAs();
          return false;
            ...
            r"^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$")
            .hasMatch(email)){
          apistatussignupvet.inputWrongEmail();
          return false;

        }
        else if(email.isEmpty){
          apistatussignupvet.inputEmptyEmail();
          return false;

        }
        else if(password1.isEmpty){
          apistatussignupvet.inputEmptyPass1();
          return false;

        }
        else if(password2.isEmpty){
          apistatussignupvet.inputEmptyPass2();
          return false;
        }
        if(password1 == password2){
          return true;
    ...
    if(SignUpVet == '2'){
      if(nameproperty.isNotEmpty ||
        ...
        if(nameproperty.isEmpty){
          apiStatusSignUpVet2.inputEmptyName();
          return false;
        }
        else if(nit.isEmpty){
          apiStatusSignUpVet2.inputEmptynit();
          return false;
        }
        else if(direction.isEmpty){
          apiStatusSignUpVet2.inputEmptyDirection();
          return false;
        }
        else if(vetCity.isEmpty){
          apiStatusSignUpVet2.inputEmptycity();
          return false;

        }
        else if(services.isEmpty){
          apiStatusSignUpVet2.inputEmptyservices();
          return false;
  ...
  bool checkLoginValidate() {
    ...
    //       r"^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$")
    //       .hasMatch(email)){
    //     apistatuslogin.inputWrongEmail();
    //     return false;
    //   }
    //   if(password1.length < 6){
    //     apistatuslogin.inputWrongPass();
    //     return false;
    //   }
    //   if (email.isNotEmpty) {
    //     if (password1.isNotEmpty) {
    //       return true;
    //     } else {
    //       apistatuslogin.inputEmptyPass();
    //       return false;
    //     }
    //   } else {
    //     apistatuslogin.inputEmptyEmail();
    //     return false;
    //   }
    // }
  ...
  Future signOut(BuildContext context) async {
    ...
    fs.clearPersistence();
    kNavigatorBack(context);
    kNavigator(context, const MyHomePage());
  }
...
abstract class ApiStatusLoginNew {
...
abstract class ApiStatusSignUp {
...
abstract class ApiStatusSignUpVet {
...
abstract class ApiStatusSignUpVet2 {
